...
## Creates Figure 1 and table 1 for manuscript
##
## contact: hofelich@umn.edu
########################################

#clear workshop
rm(list=ls())

data = read.csv(file="raw_data/data_final2.csv", header=T)
...
datalong <- data %>% 
  ...
  tidyr::pivot_longer(cols = !ResponseID, 
...
sink(file="Session_information.txt")

cat(paste("Script run on: ", Sys.Date(), "\n\n"))
sessionInfo()
